...
// Hash map to store total timer start counts
struct {
...
int trace_timer_start(void *ctx)
{
    ...
    __u64 *count = bpf_map_lookup_elem(&raw_timer_starts_total, &key);
    if (count) {
        __sync_fetch_and_add(count, 1);
    } else {
        __u64 initial_count = 1;
        bpf_map_update_elem(&raw_timer_starts_total, &key, &initial_count, BPF_NOEXIST);
    }
